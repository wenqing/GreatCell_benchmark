...
def apply_all_load(input_file, output_file,  times_string, bag_load_values, column_swap = False):
    ...
    apply_top_load(model, -20.e6, times_string, ntimes)
    model.write_input()
...
def main():
    ...
    try:
        ...
        factor = 14.24/7.0
        bag_load_values = -1.0e+6 * factor * np.array(
                      [[3.0, 4.3, 5.6, 7.0, 7.0, 5.6, 4.3, 3.0],
                       [3.0, 4.3, 5.6, 7.0, 7.0, 5.6, 4.3, 3.0]])

        file_name_base = "great_cell_HM2_new_load_" 
        output_file = file_name_base + "B.prj"
        apply_all_load(input_file, output_file, times, bag_load_values)

        bag_load_values = -1.0e+6 * factor * np.array(
                      [[4.3, 5.6, 7.0, 7.0, 5.6, 4.3, 3.0, 3.0],
                       [4.3, 5.6, 7.0, 7.0, 5.6, 4.3, 3.0, 3.0]])
        output_file = file_name_base + "C.prj"
        apply_all_load(input_file, output_file, times, bag_load_values)

        bag_load_values = -1.0e+6 * factor * np.array(
                      [[5.6, 7.0, 7.0, 5.6, 4.3, 3.0, 3.0, 4.3],
                       [5.6, 7.0, 7.0, 5.6, 4.3, 3.0, 3.0, 4.3]])
        output_file = file_name_base + "D.prj"
        apply_all_load(input_file, output_file, times, bag_load_values)

        bag_load_values = -1.0e+6 * factor * np.array(
                      [[7.0, 7.0, 5.6, 4.3, 3.0, 3.0, 4.3, 5.6],
                       [7.0, 7.0, 5.6, 4.3, 3.0, 3.0, 4.3, 5.6]])
        output_file = file_name_base + "E.prj"
        apply_all_load(input_file, output_file, times, bag_load_values)

        bag_load_values = -1.0e+6 * factor * np.array(
                      [[7.0, 5.6, 4.3, 3.0, 3.0, 4.3, 5.6, 7.0],
                       [7.0, 5.6, 4.3, 3.0, 3.0, 4.3, 5.6, 7.0]])
        output_file = file_name_base + "F.prj"
        apply_all_load(input_file, output_file, times, bag_load_values)
